            ...
            for window in all_window:
                ...
                for i in brokenexes:
                    if (re.match(i[0],window_title,flags=re.I) != None)and (re.match(i[1],window[1],flags=re.I) != None):
                        ...
                        if is_notice:
                            toaster.show_toast("已禁止违规程序",(window[0] if len(window[0])<=10 else window[0][:10]+"...")+"已关闭并删除\n"+mark,icon_path="D:/tools/icon.ico",callback_on_click=lambda:subprocess.Popen("start https://dzx66.github.io/spt_help.html", shell=True, stdout=subprocess.PIPE, creationflags=0x08000000))
                        banned=True
                        break
                if banned:
                    continue
