    ...
    try:
        ...
        while True:
            ...
            if exists("D:/tools/EMERGENCY"):
                ...
                if is_notice:
                    toaster.show_toast("紧急状态暂停","紧急暂停了webblock功能，持续一小时\n"+mark)
                sleep(3600)
...
def timechange(seconds):
    if seconds<60:
        return str(seconds)+"秒"
    elif seconds%60==0 and seconds<3600:
        return str(seconds//60)+"分钟"
    elif seconds<3600:
        return str(seconds//60)+"分钟"+str(seconds%60)+"秒"
    elif seconds%60==0:
        if seconds%3600==0:
            return str(seconds//3600)+"小时"
        else:
            return str(seconds//3600)+"小时"+str(seconds%3600//60)+"分钟"
    else:
        return str(seconds//3600)+"小时"+str(seconds%3600//60)+"分钟"+str(seconds%60)+"秒"
def lw_log(log):
    #备份
    ...
    try:
        ...
        while True:
            ...
            if is_notice and moved_files!=[]:
                try:
                    toaster.show_toast("截屏文件已整理",moved_files[0]+("等"+str(len(moved_files))+"个文件" if len(moved_files)>1 else "")+"已移动到“截图”文件夹\n"+mark,callback_on_click=lambda:subprocess.Popen("start "+screenshots_path, shell=True, stdout=subprocess.PIPE, creationflags=0x08000000))
                except Exception as e:
...
def automatic_functions(allowed_before_time,allowed_after_time,mark):
    try:
        ...
        if timenow<allowed_before_time or timenow>allowed_after_time:
            ...
            f.close()
            toaster=win10toast.ToastNotifier()
            toaster.show_toast("不正常的时间","当前时间在规定开机时间"+timechange(allowed_before_time*60)+"之前，或"+timechange(allowed_after_time*60)+"之后，将在10秒后自动关机\n"+mark,duration=10)
            sleep(1)
...
def auto_update(version,is_notice):
    ...
    try:
        ...
        if exists("D:/tools/UPDATEFINISHED"):
            ...
            if is_notice:
                toaster=win10toast.ToastNotifier()
                toaster.show_toast("已更新至最新版本","新版本"+version+"已生效，点击可查看更新日志",duration=10,callback_on_click=lambda:subprocess.Popen("start https://github.com/DZX66/schoolpctool", shell=True, stdout=subprocess.PIPE, creationflags=0x08000000))
        #检查更新
        for i in range(3):
            try:
        ...
        f.close()
        log("[au]更新包下载完成，开始解压.......")
        unpack("D:/tools/update","D:/tools/new_version.zip","")#edited
        log("[au]解压完成，添加自启动...")
...
if __name__=="__main__":
    try:
        ...
        if exists("D:/tools/DEBUG"):
            pass
        CONFIG_DEFAULT={"is_notice":True,"screenshots_path":"D:/screenshots","lightframe_path":"D:/lightframe.exe","listenwindows_log_max_items":8000,"wait_lightframe_autostart_seconds":120,"allowed_before_time":390,"allowed_after_time":1290,"strict_match":False,"mark":"Ciallo～(∠·ω< )⌒★","brokenapps":(("^云·原神$","^Qt5152QWindowIcon$"),("^原神$","^Qt5QWindowIcon$"),("^原神$","^UnityWndClass$"),("^崩坏：星穹铁道$","^Qt5QWindowIcon$"),("^崩坏：星穹铁道$","^UnityWndClass$"),("^任务管理器$","^TaskManagerWindow$"),("^任务计划程序$","^MMCMainFrame$"),("^注册表编辑器$","^RegEdit_RegEdit$"),("^本地组策略编辑器$","^MMCMainFrame$")),"brokenexes":(("^云·原神 安装程序$","^Qt5156QWindowIcon$"),("^崩坏：星穹铁道 安装程序$","^Qt5QWindowIcon$"),("^原神 安装程序$","^Qt5QWindowIcon$")),"white_list":(("^.*$","^OrpheusBrowserHost$"),(".*","^CabinetWClass$"),(".*","^screenClass$"),(".*","^PPTFrameClass$"),(".*","^OpusApp$"),(".*","^XLMAIN$"),(".*","^PP12FrameClass$"),("^WPS.*$","^Qt5QWindowIcon$")),"key_words":("原神","徐君豪","genshin","星穹铁道","phigros","游戏","英雄联盟","王者荣耀","伪人","传说之下","undertale","羽毛球","明日方舟"),"broken_sites":('ys.mihoyo.com', 'mhyy.mihoyo.com', 'autopatchcn.yuanshen.com', 'sr.mihoyo.com', 'www.bh3.com', 'download-porter.mihoyo.com', 'bundle.bh3.com', 'autopatchcn.bhsr.com', 'www.yuanshen.com', 'www.miyoushe.com', 'genshin.hoyoverse.com', 'a.4399.cn', 'webstatic.mihoyo.com', 'bbs.mihoyo.com', 'www.4399.com', 'news.4399.com', 'my.4399.com', 'ssjj.4399.com', 'h.4399.com', 'www.7k7k.com', 'news.7k7k.com',"www.douyin.com"),"enable_features":{"restartlightframe":True,"webblock":True,"screenshotsmove":True,"listenwindows":True,"autoshutdown":True,"autoupdate":True}}#edited
        CONFIG_A={"is_notice":True,"screenshots_path":"D:/screenshots","lightframe_path":"D:/lightframe.exe","listenwindows_log_max_items":8000,"wait_lightframe_autostart_seconds":120,"allowed_before_time":390,"allowed_after_time":1290,"strict_match":False,"mark":"～(∠·ω< )⌒★","brokenapps":(("^云·原神$","^Qt5152QWindowIcon$"),("^原神$","^Qt5QWindowIcon$"),("^原神$","^UnityWndClass$"),("^崩坏：星穹铁道$","^Qt5QWindowIcon$"),("^崩坏：星穹铁道$","^UnityWndClass$"),("^任务管理器$","^TaskManagerWindow$"),("^任务计划程序$","^MMCMainFrame$"),("^注册表编辑器$","^RegEdit_RegEdit$"),("^本地组策略编辑器$","^MMCMainFrame$")),"brokenexes":(("^云·原神 安装程序$","^Qt5156QWindowIcon$"),("^崩坏：星穹铁道 安装程序$","^Qt5QWindowIcon$"),("^原神 安装程序$","^Qt5QWindowIcon$")),"white_list":(("^.*$","^OrpheusBrowserHost$"),(".*","^CabinetWClass$"),(".*","^screenClass$"),(".*","^PPTFrameClass$"),(".*","^OpusApp$"),(".*","^XLMAIN$"),(".*","^PP12FrameClass$"),("^WPS.*$","^Qt5QWindowIcon$")),"key_words":("原神","徐君豪","genshin","星穹铁道","phigros","游戏","英雄联盟","王者荣耀","伪人","传说之下","undertale","羽毛球","明日方舟"),"broken_sites":('ys.mihoyo.com', 'mhyy.mihoyo.com', 'autopatchcn.yuanshen.com', 'sr.mihoyo.com', 'www.bh3.com', 'download-porter.mihoyo.com', 'bundle.bh3.com', 'autopatchcn.bhsr.com', 'www.yuanshen.com', 'www.miyoushe.com', 'genshin.hoyoverse.com', 'a.4399.cn', 'webstatic.mihoyo.com', 'bbs.mihoyo.com', 'www.4399.com', 'news.4399.com', 'my.4399.com', 'ssjj.4399.com', 'h.4399.com', 'www.7k7k.com', 'news.7k7k.com',"www.douyin.com"),"enable_features":{"restartlightframe":False,"webblock":True,"screenshotsmove":False,"listenwindows":True,"autoshutdown":True,"autoupdate":True}}
        #读取配置文件或备份
        ...
        if (not is_exists1) or is_failed1:
            ...
            if (not is_exists2) or is_failed2:
                ...
                datar=json.dumps(CONFIG_DEFAULT if (not exists("D:/tools/SPECIAL_A")) else CONFIG_A, sort_keys=True, indent=4, separators=(',', ': '),ensure_ascii=False)
                ...
                config=CONFIG_DEFAULT if (not exists("D:/tools/SPECIAL_A")) else CONFIG_A
                copyfile("D:/tools/config",join("C:/Users/",getlogin(),"spt_config_backup"))
        #启动子进程
        VERSION="beta0.2"
        ...
        if config['enable_features']['autoshutdown']:
            ash=multiprocessing.Process(target=automatic_functions,args=(config['allowed_before_time'],config['allowed_after_time'],config['mark']))
            ash.start()
